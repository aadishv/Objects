...
class Processing:
    ...
    def process_image(self, image):
        ...
        # Default values if not set - set reasonable defaults for better detection
        if self.HUE == 0:
            self.HUE = 0  # No hue shift by default
        if self.SATURATION == 0:
            self.SATURATION = 1.0  # No saturation change by default
        if self.VALUE == 0:
            self.VALUE = 1.0  # No brightness change by default
    ...
    def get_depth(self, detection: rawDetection, depth_img):
        ...
        if np.any(~np.isnan(depth_img)) and depth_img.size > 0:
            meanDepth = np.nanmean(depth_img)
        else:
            meanDepth = 0.0  # Default value when no valid depth data
            print("Warning: No valid depth data for detection")
        return meanDepth
