...
### Jetson Orin Nano
  - Requires 19V power cable
  - Run `ssh a@192.168.1.57`
  - non-root owner is `a@a-desktop`
  - All code is in `./OrinCode`
  - Code modifications (as of 04.03.25):
    1. Using regenerated .trt file for Orin's version of TensorRT
    2. Updated all TensorRT facing code to use modern APIs (coach)
    3. Lowered object thresholds
### Jetson Nano
  - Requires 5V power cable (don't fry it! :D)
  - non-root owner is `vex@vex-desktop`
  - Run `ssh vex@192.168.1.67`
  - All code is in `./JetsonCode`
  - No code modifications as of 04.03.25
### Both
  - SSH password is 2358A
  - Have Aadish's GitHub account set up (don't abuse my GitHub account!)
  - Have git on the `@aadishv/Objects` (don't ask) repo
  - Example SCP command for reference:
  ```scp JetsonExample.zip a@a-desktop:/home/vex/VAIC_24_25/JetsonExample.zip```
  - Example command to take ownership of a directory (no more `sudo`!): `sudo chown -R $USER:$USER [path]`

## Troubleshooting

**SSH says it can't find the Jetson!**

1. Firstly, make sure the wifi module is plugged in :D
2. If you're currently SSHing with the ip address (i.e., running `ssh a@192.168.1.57`), try using the non-root owner (`ssh a@a-desktop`)

*Todo for aadish: add more stuff if you have time*


# Following below is the VEX-written stuff
