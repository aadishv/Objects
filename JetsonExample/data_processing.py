...
class PostprocessYOLO(object):
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        # Create grid arrays for computing object coordinates
        # This is the fixed section that properly handles different grid dimensions
        x_range = np.arange(0, grid_w)
        y_range = np.arange(0, grid_h)
        
        # Create the grid using meshgrid instead of tile
        grid_x, grid_y = np.meshgrid(x_range, y_range)
        
        # Reshape to (grid_h, grid_w, 1, 1)
        grid_x = grid_x.reshape(grid_h, grid_w, 1, 1)
        grid_y = grid_y.reshape(grid_h, grid_w, 1, 1)
        
        # Repeat to match the number of anchors (3)
        grid_x = np.tile(grid_x, (1, 1, 3, 1))
        grid_y = np.tile(grid_y, (1, 1, 3, 1))
